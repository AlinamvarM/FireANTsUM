...
# Multi platform file locking implementation By ali Namvar
...
def get_lock():
    ...
    # Use a safe lock file path for both OSes
    lockfile = os.path.join(os.getenv("TEMP", "/tmp"), "fireants_image_lock")

    with open(lockfile, "w+") as lockhandle:
        try:
            print(f"Rank {rank} is acquiring lock")
            if sys.platform == "win32":
                # Write a byte so there's content to lock
                lockhandle.write(" ")
                lockhandle.flush()
                lockhandle.seek(0)
                # Use LK_LOCK for blocking behavior (like fcntl.LOCK_EX)
                msvcrt.locking(lockhandle.fileno(), msvcrt.LK_LOCK, 1)
            else:
                fcntl.flock(lockhandle, fcntl.LOCK_EX)
            yield
        finally:
            print(f"Rank {rank} is releasing lock")
            if sys.platform == "win32":
                lockhandle.seek(0)
                msvcrt.locking(lockhandle.fileno(), msvcrt.LK_UNLCK, 1)
            else:
